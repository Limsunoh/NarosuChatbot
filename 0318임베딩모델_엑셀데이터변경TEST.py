...
#         임베딩 = OpenAIEmbeddings(model="text-embedding-3-small", openai_api_key=API_KEY)
...
def get_embeddings(text_chunks, model_name="text-embedding-ada-002", batch_size=5):
    API_KEY = os.getenv("OPENAI_API_KEY")
...
if not os.path.exists(faiss_file_path):
    create_and_save_faiss_index("db/속성39개정제_만개데이터.xlsx")
